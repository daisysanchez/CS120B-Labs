...
unsigned char GetKeypadKey() {
	...
	if ( GetBit(~KEYPADPIN,ROW1) ) { return '1'; } //1
	if ( GetBit(~KEYPADPIN,ROW2) ) { return '4'; } //2
	if ( GetBit(~KEYPADPIN,ROW3) ) { return '7'; } //3
	if ( GetBit(~KEYPADPIN,ROW4) ) { return '*'; } //A

	// Check keys in col 2
	KEYPADPORT = SetBit(0xFF,COL2,0); // Set Px5 to 0; others 1
	asm("nop"); // add a delay to allow PORTx to stabilize before checking
	if ( GetBit(~KEYPADPIN,ROW1) ) { return '2'; } //4
	if ( GetBit(~KEYPADPIN,ROW2) ) { return '5'; } //5
	if ( GetBit(~KEYPADPIN,ROW3) ) { return '8'; } //6
	if ( GetBit(~KEYPADPIN,ROW4) ) { return '0'; } //B

	// Check keys in col 3
	KEYPADPORT = SetBit(0xFF,COL3,0); // Set Px6 to 0; others 1
	asm("nop"); // add a delay to allow PORTx to stabilize before checking
	if ( GetBit(~KEYPADPIN,ROW1) ) { return '3'; } //7
	if ( GetBit(~KEYPADPIN,ROW2) ) { return '6'; } //8
	if ( GetBit(~KEYPADPIN,ROW3) ) { return '9'; } //9
	...
	if (GetBit(~KEYPADPIN,ROW2) ) { return 'B'; } //0
	if (GetBit(~KEYPADPIN,ROW3) ) { return 'C'; } //#
	if (GetBit(~KEYPADPIN,ROW4) ) { return 'D'; } //D
